...
// src/domain/application/usecases/update-time-track/update-time-track.spec.ts
...
function compareDate(source, target) {
  const sourceDate = (0, import_dayjs.default)(new Date(source)).second(0);
  const targetDate = (0, import_dayjs.default)(new Date(target)).second(0);
...
var InMemoryTimeTrackRepository = class {
  ...
  async findManyByOwnerAndWorkspace(params) {
    const timetracks = this.items.filter((item) => item.ownerId.toString === params.ownerId && item.workspaceId.toString === params.workspaceId && (0, import_dayjs2.default)(item.registeredAt).isBetween((0, import_dayjs2.default)(params.range.start).add(1, "day"), (0, import_dayjs2.default)(params.range.end).subtract(1, "day"))).sort((a, b2) => b2.registeredAt > a.registeredAt ? -1 : 1);
    return timetracks;
  }
  async findManyByDate(params) {
    const timetracks = this.items.filter((item) => item.ownerId.toString === params.ownerId && item.workspaceId.toString === params.workspaceId && (0, import_dayjs2.default)(item.registeredAt).isSame(params.date, "day")).sort((a, b2) => b2.registeredAt > a.registeredAt ? -1 : 1);
    return timetracks;
  }
  async findById(id) {
...
describe("Update Workspace Use Case", () => {
  ...
  it("should be able to update a time track in a workspace", async () => {
    ...
    const payload = {
      description: import_faker2.faker.lorem.text(),
      registeredAt: import_faker2.faker.date.past(),
      timeTrackId: timeTrack.id.toString,
      userId: timeTrack.ownerId.toString
    };
    await sut.execute(payload);
    globalExpect(new Date(timeTrackRepository.items[0].registeredAt).getTime()).not.toEqual(new Date(oldDate).getTime());
  });
  it("should not be able to update a time track with a future date", async () => {
    ...
    const payload = {
      description: import_faker2.faker.lorem.text(),
      registeredAt: import_faker2.faker.date.soon(),
      timeTrackId: timeTrack.id.toString,
      userId: timeTrack.ownerId.toString
    };
    const result = await sut.execute(payload);
    globalExpect(result.isLeft()).toBe(true);
    globalExpect(result.value).toBeInstanceOf(TimeUpdateToFutureDateError);
  });
  it("should not be able to update a time track that doesnt exists", async () => {
    const payload = {
      description: import_faker2.faker.lorem.text(),
      registeredAt: import_faker2.faker.date.past(),
      timeTrackId: import_faker2.faker.string.uuid(),
      userId: import_faker2.faker.string.uuid()
    };
    const result = await sut.execute(payload);
    globalExpect(result.isLeft()).toBe(true);
    globalExpect(result.value).toBeInstanceOf(ResourceNotFoundError);
  });
  it("should not be able to update a time track if user id diff", async () => {
    ...
    const payload = {
      ...
      registeredAt: import_faker2.faker.date.past(),
      timeTrackId: timeTrack.id.toString,
      userId: (0, import_crypto2.randomUUID)()
    };
    const result = await sut.execute(payload);
    globalExpect(result.isLeft()).toBe(true);
    globalExpect(result.value).toBeInstanceOf(NotAllowedError);
  });
  it("should not be able to update a time track if there is another time track with the same register time ", async () => {
    const existingDate = import_faker2.faker.date.past();
    ...
    const result = await sut.execute(payload);
    globalExpect(result.isLeft()).toBe(true);
    globalExpect(result.value).toBeInstanceOf(DateConflictError);
  });
});
