...
// src/domain/application/usecases/create-company/create-company.spec.ts
...
var CreateCompanyUseCase = class {
  constructor(companyRepository2, managerRepository2) {
    this.companyRepository = companyRepository2;
    this.managerRepository = managerRepository2;
  }
  async execute({
    name,
    managerId
...
// test/repositories/in-memory-manager-repository.ts
var InMemoryManagerRepository = class {
  constructor() {
    this.items = [];
  }
...
describe("Create Company Use Case", () => {
  ...
  it("should be able to create a company", async () => {
    const managerId = new UniqueId(import_faker.faker.string.uuid());
    ...
    globalExpect(companyRepository.items[0].id).toEqual(globalExpect.any(UniqueId));
    globalExpect(companyRepository.items[0].managerId.toString).toEqual(managerId.toString);
  });
  it("should not be able to create a company without a valid manager", async () => {
    const payload = {
      name: import_faker.faker.company.name(),
      managerId: (0, import_crypto.randomUUID)()
    };
