...
// src/domain/application/usecases/create-manager/create-manager.spec.ts
...
var CreateManagerUseCase = class {
  constructor(managerRepository2, hasher2) {
...
describe("Create Manager Use Case", () => {
  ...
  it("should be able to create a manager", async () => {
    ...
    globalExpect(managerRepository.items[0].id).toEqual(globalExpect.any(UniqueId));
  });
  it("should not be able to create a manager with an existing email", async () => {
    ...
    globalExpect(result.value).toBeInstanceOf(ResourceAlreadyExistsError);
  });
});
