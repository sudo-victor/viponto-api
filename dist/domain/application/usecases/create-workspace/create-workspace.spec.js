...
// src/domain/application/usecases/create-workspace/create-workspace.spec.ts
...
var CreateWorkspaceUseCase = class {
  constructor(workspaceRepository2, companyRepository2) {
...
describe("Create Workspace Use Case", () => {
  ...
  it("should be able to create a workspace", async () => {
    const managerId = new UniqueId((0, import_crypto.randomUUID)());
    const company = makeCompany({ managerId });
    await companyRepository.create(company);
    const payload = {
      name: import_faker2.faker.person.fullName(),
    ...
    globalExpect(workspaceRepository.items[0].id).toEqual(globalExpect.any(UniqueId));
  });
  it("should not be able to create a workspace if company was not found", async () => {
    const payload = {
      name: import_faker2.faker.person.fullName(),
      description: import_faker2.faker.lorem.text(),
      companyId: (0, import_crypto.randomUUID)(),
      periodType: "hour",
      value: 50,
