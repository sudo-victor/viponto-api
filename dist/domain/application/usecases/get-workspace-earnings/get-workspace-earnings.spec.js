...
var GetWorkspaceEarnings = class {
  constructor(workspaceRepository2, timeTracksRepository) {
    this.workspaceRepository = workspaceRepository2;
...
// test/repositories/in-memory-workspace-repository.ts
var InMemoryWorkspaceRepository = class {
  constructor() {
    this.items = [];
  }
...
function compareDate(source, target) {
  const sourceDate = (0, import_dayjs2.default)(new Date(source)).second(0);
  const targetDate = (0, import_dayjs2.default)(new Date(target)).second(0);
...
var InMemoryTimeTrackRepository = class {
  ...
  async findManyByOwnerAndWorkspace(params) {
    const timetracks = this.items.filter((item) => item.ownerId.toString === params.ownerId && item.workspaceId.toString === params.workspaceId && (0, import_dayjs3.default)(item.registeredAt).isBetween((0, import_dayjs3.default)(params.range.start).add(1, "day"), (0, import_dayjs3.default)(params.range.end).subtract(1, "day"))).sort((a, b2) => b2.registeredAt > a.registeredAt ? -1 : 1);
    return timetracks;
  }
  async findManyByDate(params) {
    const timetracks = this.items.filter((item) => item.ownerId.toString === params.ownerId && item.workspaceId.toString === params.workspaceId && (0, import_dayjs3.default)(item.registeredAt).isSame(params.date, "day")).sort((a, b2) => b2.registeredAt > a.registeredAt ? -1 : 1);
    return timetracks;
  }
  async findById(id) {
...
// test/factories/make-workspace.ts
var import_faker = require("@faker-js/faker");
var import_crypto = require("crypto");
async function makeWorkspace({ id }) {
  return Workspace.create({
    name: import_faker.faker.lorem.word(),
    description: import_faker.faker.lorem.text(),
    company_id: new UniqueId((0, import_crypto.randomUUID)()),
    value: new WorkspaceValue({ period_type: "hour", value: 50 })
  }, id);
}

// src/domain/application/usecases/get-workspace-earnings/get-workspace-earnings.spec.ts
...
async function makeTimeTrack({ id, registered_at, owner_id, workspace_id }) {
  return TimeTrack.create({
    owner_id: owner_id ?? new UniqueId((0, import_crypto2.randomUUID)()),
    workspace_id: workspace_id ?? new UniqueId((0, import_crypto2.randomUUID)()),
...
describe("Get Workspace Earnings Use Case", () => {
  ...
  it("should be able to return the total earnings in current month", async () => {
    ...
    const result = await sut.execute({
      ...
      endRange: /* @__PURE__ */ new Date()
    });
    globalExpect(result.isRight()).toBe(true);
    globalExpect(result.value.total).toEqual(75);
  });
  it("should not be able to return the earnings if workspace not found", async () => {
    const result = await sut.execute({
      workspaceId: (0, import_crypto3.randomUUID)(),
      userId: (0, import_crypto3.randomUUID)(),
      startRange: new Date(2023, 10, 1, 0, 0, 0),
      endRange: /* @__PURE__ */ new Date()
    });
