...
// src/domain/application/usecases/register-time-track/register-time-track.spec.ts
...
describe("Create Workspace Use Case", () => {
  ...
  it("should be able to register a time track in a workspace", async () => {
    ...
    const payload = {
      ownerId: (0, import_crypto.randomUUID)(),
      workspaceId: workspace.id.toString,
      description: import_faker.faker.lorem.text()
    };
    await sut.execute(payload);
    globalExpect(timeTrackRepository.items[0].id).toEqual(globalExpect.any(UniqueId));
  });
