...
// src/domain/application/usecases/create-employee/create-employee.spec.ts
...
var CreateEmployeeUseCase = class {
  constructor(employeeRepository2, companyRepository2, hasher2) {
    this.employeeRepository = employeeRepository2;
    this.companyRepository = companyRepository2;
    this.hasher = hasher2;
  }
  async execute({
    name,
...
// test/repositories/in-memory-employee-repository.ts
var InMemoryEmployeeRepository = class {
  constructor() {
    this.items = [];
  }
...
// test/repositories/in-memory-company-repository.ts
var InMemoryCompanyRepository = class {
  ...
  async findManyByManagerId(managerId) {
    const companies = this.items.filter((item) => item.managerId.toString === managerId);
    return companies;
  }
  async findById(id) {
...
// test/factories/make-manager.ts
...
async function makeManager({ id }) {
  return Manager.create({
    ...
    password_hash: await (0, import_bcryptjs.hash)(import_faker.faker.internet.password(), 6)
  }, id);
}
...
// test/factories/make-company.ts
var import_faker2 = require("@faker-js/faker");
function makeCompany({ id, managerId }) {
  return Company.create({
    name: import_faker2.faker.company.name(),
...
var BcryptHasher = class {
  async hash(plain) {
    return await (0, import_bcryptjs2.hash)(plain, 6);
  }
  async compare(plain, hash3) {
    return await (0, import_bcryptjs2.compare)(plain, hash3);
  }
};
...
describe("Create Employee Use Case", () => {
  ...
  it("should be able to create a employee", async () => {
    ...
    const payload = {
      name: import_faker3.faker.person.fullName(),
      companyId: company.id.toString,
      managerId: manager.id.toString,
      password: import_faker3.faker.internet.password()
    };
    await sut.execute(payload);
    globalExpect(employeeRepository.items[0].id).toEqual(globalExpect.any(UniqueId));
  });
  it("should not be able to create a employee if company not found", async () => {
    const payload = {
      name: import_faker3.faker.person.fullName(),
      companyId: import_faker3.faker.string.uuid(),
      managerId: import_faker3.faker.string.uuid(),
      password: import_faker3.faker.internet.password()
    };
  ...
  it("should not be able to create a employee if manager is not an admin of the current company", async () => {
    const company = makeCompany({ managerId: new UniqueId(import_faker3.faker.string.uuid()) });
    companyRepository.create(company);
    const payload = {
      name: import_faker3.faker.person.fullName(),
      companyId: company.id.toString,
