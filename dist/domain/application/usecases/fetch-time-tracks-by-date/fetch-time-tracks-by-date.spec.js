...
// src/domain/application/usecases/fetch-time-tracks-by-date/fetch-time-tracks-by-date.spec.ts
...
describe("Fetch Time Tracks By Date Use Case", () => {
  ...
  it("should be able to fetch time tracks by date", async () => {
    ...
    globalExpect(result.value?.hours).toEqual(THIRTY_MIN_IN_HOURS);
    globalExpect(result.value?.timeTracks).toHaveLength(3);
  });
});
