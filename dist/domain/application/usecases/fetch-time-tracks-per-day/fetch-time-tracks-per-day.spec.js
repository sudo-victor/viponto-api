...
var FetchTimeTracksPerDayUseCase = class {
  constructor(timeTrackRepository2) {
    this.timeTrackRepository = timeTrackRepository2;
  }
  async execute({
    userId,
    workspaceId,
...
// src/core/utils/compare-date.ts
var import_dayjs3 = __toESM(require("dayjs"));
function compareDate(source, target) {
  const sourceDate = (0, import_dayjs3.default)(new Date(source)).second(0);
  const targetDate = (0, import_dayjs3.default)(new Date(target)).second(0);
  return sourceDate.isSame(targetDate);
}

// test/repositories/in-memory-time-track-repository.ts
var import_dayjs4 = __toESM(require("dayjs"));
var import_isBetween = __toESM(require("dayjs/plugin/isBetween"));
import_dayjs4.default.extend(import_isBetween.default);
var InMemoryTimeTrackRepository = class {
  constructor() {
    this.items = [];
  }
  async create(timetrack) {
    this.items.push(timetrack);
  }
  async findByOwnerAndTime({ ownerId, time }) {
    const timetrack = this.items.filter((item) => item.ownerId.toString === ownerId && compareDate(item.registeredAt, time));
    if (timetrack.length === 0) {
      return null;
    }
    return timetrack[0];
  }
  async findManyByOwnerAndWorkspace(params) {
    const timetracks = this.items.filter((item) => item.ownerId.toString === params.ownerId && item.workspaceId.toString === params.workspaceId && (0, import_dayjs4.default)(item.registeredAt).isBetween((0, import_dayjs4.default)(params.range.start).add(1, "day"), (0, import_dayjs4.default)(params.range.end).subtract(1, "day"))).sort((a, b2) => b2.registeredAt > a.registeredAt ? -1 : 1);
    return timetracks;
  }
  async findManyByDate(params) {
    const timetracks = this.items.filter((item) => item.ownerId.toString === params.ownerId && item.workspaceId.toString === params.workspaceId && (0, import_dayjs4.default)(item.registeredAt).isSame(params.date, "day")).sort((a, b2) => b2.registeredAt > a.registeredAt ? -1 : 1);
    return timetracks;
  }
  async findById(id) {
...
// test/factories/make-time-track.ts
var import_faker = require("@faker-js/faker");
var import_crypto = require("crypto");
async function makeTimeTrack({ id, registered_at, owner_id, workspace_id }) {
  return TimeTrack.create({
    owner_id: owner_id ?? new UniqueId((0, import_crypto.randomUUID)()),
    workspace_id: workspace_id ?? new UniqueId((0, import_crypto.randomUUID)()),
    registered_at: registered_at ?? /* @__PURE__ */ new Date(),
    description: import_faker.faker.lorem.text()
  }, id);
}

// src/domain/application/usecases/fetch-time-tracks-per-day/fetch-time-tracks-per-day.spec.ts
var import_crypto2 = require("crypto");
var import_dayjs5 = __toESM(require("dayjs"));
var timeTrackRepository;
var sut;
describe("Fetch Time Tracks Per Day Use Case", () => {
  ...
  it("should be able to fetch time tracks per day", async () => {
    const ownerId = new UniqueId((0, import_crypto2.randomUUID)());
    const workspaceId = new UniqueId((0, import_crypto2.randomUUID)());
    const firstDate = /* @__PURE__ */ new Date();
    const secondDate = (0, import_dayjs5.default)(firstDate).subtract(1, "days").toDate();
    for (let i = 1; i <= 5; i++) {
    ...
    const expectedTimeTracksKeys = [
      (0, import_dayjs5.default)(secondDate).second(0).format("YYYY-MM-DD"),
      (0, import_dayjs5.default)(firstDate).second(0).format("YYYY-MM-DD")
    ].sort((a, b2) => a > b2 ? 1 : -1);
    globalExpect(Object.keys(result.value?.timeTracks)).toHaveLength(2);
    globalExpect(Object.keys(result.value?.timeTracks)).toEqual(expectedTimeTracksKeys);
  });
  it("should be able to fetch time tracks per day between today and five days ago", async () => {
    const ownerId = new UniqueId((0, import_crypto2.randomUUID)());
    const workspaceId = new UniqueId((0, import_crypto2.randomUUID)());
    const firstDate = /* @__PURE__ */ new Date();
    const secondDate = (0, import_dayjs5.default)(firstDate).subtract(4, "days").toDate();
    const thirdDate = (0, import_dayjs5.default)(firstDate).subtract(5, "days").toDate();
    for (let i = 1; i < 5; i++) {
      timeTrackRepository.create(await makeTimeTrack({
        registered_at: i % 2 ? firstDate : secondDate,
        owner_id: ownerId,
  ...
  it("should be able to fetch time tracks per day between two dates", async () => {
    const ownerId = new UniqueId((0, import_crypto2.randomUUID)());
    const workspaceId = new UniqueId((0, import_crypto2.randomUUID)());
    const firstDate = (0, import_dayjs5.default)(/* @__PURE__ */ new Date()).second(0).subtract(10, "days").toDate();
    const secondDate = (0, import_dayjs5.default)(firstDate).subtract(20, "days").toDate();
    const thirdDate = (0, import_dayjs5.default)(firstDate).subtract(21, "days").toDate();
    for (let i = 1; i <= 5; i++) {
      timeTrackRepository.create(await makeTimeTrack({
        registered_at: i % 2 ? firstDate : secondDate,
        owner_id: ownerId,
