...
// src/domain/application/usecases/authenticate/authenticate.spec.ts
...
var AuthenticateUseCase = class {
  constructor(managerRepository2, encrypter2, hasher2) {
...
var envSchema = import_zod.default.object({
  SECRET_KEY: import_zod.default.string(),
  JWT_PUBLIC_KEY: import_zod.default.string(),
  JWT_PRIVATE_KEY: import_zod.default.string()
});
...
// src/infra/gateways/jwt-encrypter.ts
...
var BcryptHasher = class {
  ...
  async compare(plain, hash3) {
    return await (0, import_bcryptjs.compare)(plain, hash3);
  }
};
...
describe("Authenticate Use Case", () => {
  ...
  it("should be able to create a session for a manager", async () => {
    const payload = {
      ...
      password_hash: import_faker.faker.internet.password()
    };
    managerRepository.create(Manager.create({ ...payload, password_hash: await (0, import_bcryptjs2.hash)(payload.password_hash, 6) }));
    const result = await sut.execute({
      email: payload.email,
      password: payload.password_hash
    });
    globalExpect(result.isRight()).toBe(true);
    globalExpect(result.value).toEqual(globalExpect.any(Object));
  });
  it("should not be able to create a session to an unexisting manager", async () => {
    ...
    globalExpect(result.value).toBeInstanceOf(InvalidCredentialsError);
  });
